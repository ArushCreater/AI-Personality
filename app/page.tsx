    ...
    <div className="flex flex-col h-screen bg-black">
        ...
        <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="flex-1 min-w-0 space-y-2 group">
                <div className={`prose prose-invert max-w-none ${
                  ...
                  <div className={`inline-block text-left ${
                    ...
                    <p className="text-[15px] leading-relaxed text-white/90 whitespace-pre-wrap break-words m-0 animate-text-appear">
                      {message.content}
                    </p>
                  </div>
                </div>
                ...
                <div className={`flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200 ${
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-white/10 active:bg-white/15 transition-all duration-200 hover:scale-110 active:scale-95"
                    title="Copy to clipboard"
                  >
                    <svg className="w-3.5 h-3.5 text-white/50 hover:text-white/80 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isLoading && (
            <div className="flex gap-4 animate-slide-up">
              <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-gradient-to-br from-white/10 to-white/5 border border-white/10 flex items-center justify-center animate-pulse-glow">
                <svg className="w-4 h-4 text-white/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <div className="flex-1 pt-1">
                <div className="flex space-x-1.5">
                  <div className="w-2 h-2 bg-white/30 rounded-full animate-bounce-smooth"></div>
                  <div className="w-2 h-2 bg-white/30 rounded-full animate-bounce-smooth" style={{ animationDelay: "0.15s" }}></div>
                  <div className="w-2 h-2 bg-white/30 rounded-full animate-bounce-smooth" style={{ animationDelay: "0.3s" }}></div>
                </div>
              </div>
            </div>
          )}
      ...
      {showScrollButton && (
        <button
          ...
          className="fixed bottom-32 left-1/2 -translate-x-1/2 p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 hover:scale-110 active:scale-95 transition-all duration-300 animate-bounce-in shadow-xl shadow-white/10"
        >
      ...
      {/* Input Area */}
      <div className="sticky bottom-0 px-6 py-6 bg-gradient-to-t from-black via-black/95 to-transparent pointer-events-none">
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto pointer-events-auto">
          <div className="flex items-center gap-3 bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl px-2 py-2 focus-within:border-white/20 transition-all duration-300 hover:border-white/15">
            <textarea
              ref={textareaRef}
              value={input}
              ...
              disabled={isLoading}
              rows={1}
              className="flex-1 bg-transparent text-white text-[15px] placeholder-white/40 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed resize-none max-h-32 py-2.5 px-4 leading-6"
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="flex-shrink-0 p-2.5 rounded-2xl bg-white text-black hover:bg-white/90 focus:outline-none disabled:opacity-20 disabled:cursor-not-allowed disabled:hover:bg-white transition-all duration-300 hover:scale-105 active:scale-95 disabled:hover:scale-100"
            >
              {isLoading ? (
